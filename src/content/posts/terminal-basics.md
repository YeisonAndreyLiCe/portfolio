...
- `echo $PATH`: /usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:
   ...
   *the directories where the shell looks for commands*
...
find . -name "*.txt" -exec grep "foo" {} \; : find files
with .txt extension and grep for "foo"
find . -name "*.txt" -exec grep "foo" {} \; -print : find files with .txt
extension and grep for "foo", print results
find . -name "*.txt" -exec grep "foo" {} \; -print -exec rm {} \; : find
